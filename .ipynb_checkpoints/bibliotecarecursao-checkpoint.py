...
#             print(args[i+1],"\n")
    ...
    def durand_kerner (args) :
        ...
        def diff_lists (list1, list2):
            diff = [abs(val - list2[i]) for i, val in enumerate(list1)]
            return diff

        precisao = 1e-12
        it = 0
        rootNum = len(args)-1
        roots = [complex(0.4,0.9)+complex(i/10,-i/5) for i in range(rootNum)]
        roots_old = [complex(0,0) for i in range(rootNum)]

        while (any(x > precisao for x in diff_lists(roots, roots_old))):
            roots_old = roots.copy()
            for i, val in enumerate(roots_old):
                roots[i] = roots_old[i] - f(roots_old[i], args)/den(i, roots_old)
            it += 1
